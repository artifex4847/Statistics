...
if _N_<1 then delete;
		 ...
		 logprice=log(price);
		 sq=t*t;
		 x=log(price)-3.1029+0.006567*t-0.0000418*sq;
run;
...
title 'Time plot of oil price Xt';
run;

proc arima data=TS9.example93;
 identify var=x scan minic center esacf stationarity=(adf=(0,1,2,3,4));
run;
...
price=exp(3.1029-0.006567*t+0.00004188sq+x);
ll95=exp(l95);
uu95=exp(u95);
forecast1=exp(3.1029-0.006567*t+0.00004188sq+forecast+std*std/2);
run;
