...
Data TS0.example01_shmonth;
         infile 'C:\Data_cru\SH_month.dat';
         input temperature @@;
         date = intnx( 'month', '1jan1850'd, _N_); 
         format date monyy.; 
	     char = date; 
		 ...
		 t=_N_;
run;
Data TS0.example01_s;
set TS0.example01_shmonth;
if _N_<121 then delete;
run;
Proc gplot data= TS0.example01_s; 
      symbol i=joint v= h=1; 
      plot temperature * date;
      title 'South monthly temperature from 1900 to 2012';
   run;

Proc glm data=TS0.example01_s; 
class month; 
